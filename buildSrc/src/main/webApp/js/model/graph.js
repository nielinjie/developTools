(function() {
    ...
    var tip = d3.tip().attr('class', 'd3-tip').offset([ -5, 0 ]).html(
            function(n) {
                return toolbar(n)
            })
    function toolbar(n) {
        var button = $("<button/>").text('Focus. ').attr('type', 'button')
                .addClass('btn btn-node-focus btn-default').attr(
    ...
    window.initGraph = function() {
        ...
        force = d3.layout.force().size([ fwidth - 30, fheight - 30 ]).charge(
                -1000).on("tick", tick).linkDistance(function(l) {
            return distance[l.type]
        })

        function tick() {

            var link = svg.select(".viewport").selectAll(".link"), node = svg
                    .select(".viewport").selectAll(".node");
            link.attr('d', function(d) {

                var deltaX = d.target.x - d.source.x, deltaY = d.target.y
                        - d.source.y, dist = Math.sqrt(deltaX * deltaX + deltaY
                        * deltaY), normX = deltaX / dist
                normY = deltaY / dist
                sourcePadding = 20, targetPadding = 20, sourceX = d.source.x
        ...
        svg.append('marker').attr('id', 'arrow').attr('viewBox', '0 0 10 10')
                .attr('refX', 10).attr('refY', 5).attr('markerWidth', 10).attr(
                        'markerHeight', 10).attr('orient', 'auto').append(
                        'path').attr('d', 'M 0 10 L 10 5 L 0 0').attr('style',
        ...
        svg.append('marker').attr('id', 'triangle')
                ...
                .attr('markerWidth', 10).attr('markerHeight', 10).attr(
                        ...
                        'M 9 0 L 0 5 L 9 10 L 9 0');

        svg.append('marker').attr('id', 'include').attr('viewBox', '0 0 10 10')
                .attr('refX', 0).attr('refY', 5).attr('markerWidth', 10).attr(
                        'markerHeight', 10).attr('orient', 'auto').append(
                        'text').text("include")

        panZoomTiger = svgPanZoom('.svg svg', {
            minZoom : 0.2,
            onZoom : function(e) {
                d3.selectAll('text').style('visibility',
                        e < 0.8 ? 'hidden' : 'visible')
                graphZoom = e
            }
        })
    }
    ...
    window.update = function(graph) {
        ...
        var fnname = _(force.nodes()).pluck("name")
        var removedNames = _(fnname).difference(_(graph.nodes).pluck("name"))
        var addNodes = _(graph.nodes).filter(function(gn) {
            return !_(fnname).contains(gn.name)
        })
        var existedNodes = _(force.nodes()).reject(function(n) {
            return _(removedNames).contains(n.name)
        })

        var allNode = _(existedNodes).union(addNodes)
        var allLink = _(graph.links).map(function(l) {
            return _(l).extend({
                source : _(allNode).findWhere({
                    name : l.source.name
                }),
                target : _(allNode).findWhere({
                    name : l.target.name
                })
            })
        })
        force.nodes(allNode).links(allLink)

        link = link.data(force.links(), function(l) {
            return "" + l.source.name + l.target.name + l.type
        })

        link.enter().append("path").attr("class", function(l) {
            return "link " + l.type
        }).attr('marker-start', function(l) {
            return l.type == 'g' ? 'url(#triangle)' : ''
        }).attr('marker-end', function(l) {
            return l.type != 'g' ? 'url(#arrow)' : ''
        })
        link.exit().remove()

        node = node.data(force.nodes(), function(n) {
            return n.name
        })
        var nodeE = node.enter()
        node.exit().remove()

        var nodeG = nodeE.append("g").attr("class", "node").attr("data-name",
                function(n) {
                    return n.name
        ...
        nodeG.append(
                function(n) {
                    return (n.type == "function") ? document.createElementNS(
    ...
    window.filter = function(domain, filterFun) {
        ...
        graph.nodes = []
        graph.links = []
        _(domain.functions).each(function(f) {
            graph.nodes.push({
                name : f.name,
                type : "function"
            })
        })
        _(domain.entities).each(function(e) {
            graph.nodes.push({
                name : e.name,
                type : "entity"
            })
        })
        _(domain.refs).each(function(r) {
            graph.links.push({
